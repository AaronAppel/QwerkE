...
namespace QwerkE {
	...
	namespace Helpers {
		...
		uPtr<char[]> NumberAppendOrIncrement(const char* const charArray)
		{
			...
			uPtr<char[]> returnCharArray = std::make_unique<char[]>(returnCharArrayLength + 1);
			memcpy(returnCharArray.get(), charArray, charArrayLength);
			returnCharArray.get()[returnCharArrayLength] = '\0';
			...
			for (size_t i = returnCharArrayLength - digits; i < returnCharArrayLength; i++)
			{
				...
				returnCharArray.get()[i] = NumberIntToChar(newCharValue);
			}
