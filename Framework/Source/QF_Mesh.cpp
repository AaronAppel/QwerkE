...
namespace QwerkE {
    ...
    {
        ASSERT(data, "MeshData is null!");
        ASSERT(m_BufferData.numPositions == 0, "Mesh already has vertex data!");

        m_BufferData = data->BufferInfo();

        ASSERT(m_BufferData.numPositions > 0, "Error buffering mesh vertex data!");
