...
namespace QwerkE {
	...
	public:
		...
		{
			return m_Scene->Registry().emplace<T>(m_EnttId, std::forward<Args>(args)...);
		}

		template <typename T>
		bool HasComponent() const
		{
			return m_Scene->Registry().has<T>(m_EnttId);
		}

		template <typename T>
		T& GetComponent()
		{
			return m_Scene->Registry().get<T>(m_EnttId);
		}
		...
		void RemoveComponent()
		{
			...
			m_Scene->Registry().remove<T>(m_EnttId);
		}
	...
	private:
			...
			m_Scene(scene)
		{
			...
			m_EnttId = m_Scene->Registry().create();
