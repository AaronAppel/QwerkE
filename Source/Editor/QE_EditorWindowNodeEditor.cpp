...
#include "QF_ComponentHelpers.h" // #TODO Remove test code
#include "QF_Sound.h" // #TODO Remove test code
#include "Libraries/imspinner/imspinner.h" // #TODO Remove test code
...
namespace QwerkE {
	...
	void EntityComponent(const EntityHandle& a_Entity)
	{
		([&]() {

			if (!a_Entity.HasComponent<Component>())
			{
				return;
			}

			auto my_node_id = uniqueId++;
			ed::BeginNode(my_node_id);
			const Mir::TypeInfo* typeInfo = Mir::InfoForType<Component>();
			ImGui::Text(typeInfo->stringName.data());
			for (size_t i = 0; i < typeInfo->fields.size(); i++)
			{
				ed::PinKind pinKind = i < 1 ? ed::PinKind::Input : ed::PinKind::Output;
				if (ed::PinKind::Output == pinKind)
				{
					ImGui::Dummy({ 35.f, 0.f });
					ImGui::SameLine();
				}
				ed::BeginPin(uniqueId++, pinKind);
				std::string a = "";

				if (ed::PinKind::Input == pinKind)
				{
					a = "-> ";
				}
				a += typeInfo->fields[i].name;
				if (ed::PinKind::Output == pinKind)
				{
					a += " ->";
				}
			...
			if (firstframe)
			{
				ed::SetNodePosition(my_node_id, ImVec2(430, 40));
			}
		}(), ...);
	}

	template<typename... Components>
	void EntityComponents(TemplateArgumentList<Components...>, const EntityHandle& a_Entity)
	{
		EntityComponent<Components...>(a_Entity);
	}
	...
	namespace Editor {
		...
		// #TODO Avoid static. Move to window class to support unique instancing
		...
		void local_WidgetsNodeEditor(ax::NodeEditor::EditorContext* context)
		{
			...
			for (auto& linkInfo : m_Links)
			{
				if (true)
				{
					// #TODO Enable flowing animations
					// ed::Flow(linkInfo.Id);
				}
			}
			...
			// My nodes - Start
			if (EntityHandle entity = Scenes::GetCurrentScene()->GetCurrentCameraEntity())
			{
				EntityComponents(EntityComponentsList{}, entity);
			}

			static int constIntValue = 0;
			{
				float _width = 100.f;
				ed::BeginNode(uniqueId++);
				ImGui::Text("Const");

				// ImGui::Dummy({ _width * 0.5f, 0.f });
				ImGui::SetNextItemWidth(75.f);
				ImGui::InputInt("##ConstIntValue", &constIntValue);
				ImGui::SameLine();
			...
			{
				float _width = 100.f;
				int thisNodeId = uniqueId++;
				ed::BeginNode(uniqueId++);
				ImGui::Text("Event");
				int thisPinId = uniqueId++;

				// ImGui::Dummy({ _width * 0.5f, 0.f });
				static int event1Id = 0;
				ImGui::SetNextItemWidth(60.f);
				ImGui::SliderInt("Id##Event1Id", &event1Id, 1, 100);
				ImGui::SameLine();
				ImGui::SetNextItemWidth(60.f);
				ed::BeginPin(thisPinId++, ed::PinKind::Output);
				ImGui::Text("Out ->");
				ed::EndPin();
				ed::EndNode();
			}

			int funNodeId = uniqueId++;
			int funIntValuePinId = 0;
			static int funIntValue = 0;
			{
				ed::BeginNode(funNodeId);
				ImGui::Text("Fun");
				...
				float _width = 75.f;
				funIntValuePinId = uniqueId++;
				ed::BeginPin(funIntValuePinId, ed::PinKind::Input);
				ImGui::SetNextItemWidth(_width);
				ImGui::InputInt("##FunIntValue", &funIntValue, 1, 100, 0);
				ed::EndPin();

				ImGui::SameLine();
				ImGui::SetNextItemWidth(_width);
				static int scrollerInt = 0;
				ImGui::ScrollerInt("ScrollerInt", &scrollerInt, 1, 100, 0); // #TODO Fights with graph view scroll
				if (ImGui::IsItemHovered())
				{
					// #TODO Don't scroll/zoom graph view
				}
...

				{
					...
					ImSpinner::SpinnerThickToSin("SpinnerThickToSin2", 16, 1, white, 2.f, 20, 6);
				}
				ed::EndNode();
			}
			// My nodes - End
			...
			for (auto& linkInfo : m_Links)
			{
				ed::Link(linkInfo.Id, linkInfo.InputId, linkInfo.OutputId, ImColor(255, 180, 10, 255), 1.0f);
			}
			...
			// ==================================================================================================
			// Linked state and logic updating
			for (auto& linkInfo : m_Links)
			{
				if (linkInfo.InputId == ed::PinId(funIntValuePinId) ||
					linkInfo.OutputId == ed::PinId(funIntValuePinId))
				{
					// #TODO Get value from output pin (const node) and set on input pin (fun node)
					// funNodeId
					funIntValue = constIntValue;
				}
			}
