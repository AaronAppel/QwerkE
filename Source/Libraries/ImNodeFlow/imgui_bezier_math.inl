...
inline ImRect ImCubicBezierBoundingRect(const ImVec2& p0, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3)
{
    ...
    auto c = ImVec2(3.0f * p1.x - 3.0f * p0.x,
                    3.0f * p1.y - 3.0f * p0.y);
    auto tmp = ImMul(a, c);
    auto delta_squared = ImVec2(ImMul(b, b).x - 4.0f * tmp.x, ImMul(b, b).y - 4.0f * tmp.y);
...
inline ImCubicBezierIntersectResult ImCubicBezierLineIntersect(const ImVec2& p0, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& a0, const ImVec2& a1)
{
    ...
    auto c3 = ImVec2(-p0.x + 3.0f * p1.x - 3.0f * p2.x + p3.x,
                      -p0.y + 3.0f * p1.y - 3.0f * p2.y + p3.y);
    auto c2 = ImVec2(3.0f * p0.x - 6.0f * p1.x + 3.0f * p2.x,
                      3.0f * p0.y - 6.0f * p1.y + 3.0f * p2.y);
    auto c1 = ImVec2(-3.0f * p0.x + 3.0f * p1.x,
                      -3.0f * p0.y + 3.0f * p1.y);
    auto c0 = ImVec2(p0.x, p0.y);
