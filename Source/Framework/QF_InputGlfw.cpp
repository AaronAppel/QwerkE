...
extern void InternalInitialize();
extern void InternalNewFrame();
extern void InternalShutdown();

extern void InternalKeyChanged(const QKey aKey, const QKeyState aKeyState);

extern void InternalMouseMove(const vec2f& aNewPosition);
extern void InternalMouseButton(const QKey aKey, const QKeyState aKeyState);
extern void InternalMouseScroll(const double aOffsetX, const double aOffsetY);
...
static void AddGamepad(const u8 aGamepadId) {
	...
	int axesCount;
	const float* axes = glfwGetJoystickAxes(aGamepadId, &axesCount);
	sDeviceAxesStates.emplace_back(0.f);
	for (size_t i = 0; i < axesCount; i++) {
		sDeviceAxesStates[sDeviceAxesStates.size() - 1].emplace_back(axes[i]);
	}
...
static void RemoveGamepad(const u8 aGamepadId) {
	for (size_t i = 0; i < sGamepadIds.size(); i++) {
		if (aGamepadId == sGamepadIds[i]) {
			...
			for (size_t i = 0; i < sGamepadAxisLeftStickBuffers.size(); i++) {
				if (aGamepadId == sGamepadAxisLeftStickBuffers[i].first) {
					sGamepadAxisLeftStickBuffers.erase(sGamepadAxisLeftStickBuffers.begin() + i);
				}
			}
			for (size_t i = 0; i < sGamepadAxisRightStickBuffers.size(); i++) {
				if (aGamepadId == sGamepadAxisRightStickBuffers[i].first) {
					sGamepadAxisRightStickBuffers.erase(sGamepadAxisRightStickBuffers.begin() + i);
				}
			}
			for (size_t i = 0; i < sGamepadAxisTriggersBuffers.size(); i++) {
				if (aGamepadId == sGamepadAxisTriggersBuffers[i].first) {
					sGamepadAxisTriggersBuffers.erase(sGamepadAxisTriggersBuffers.begin() + i);
				}
			}
...
void NewFrame() {
	...
	for (size_t i = 0; i < sGamepadIds.size(); i++) {
		...
		ASSERT(glfwJoystickPresent(sGamepadIds[i]), "Device no longer present!");

		int axesCount;
		const float* axes = glfwGetJoystickAxes(sGamepadIds[i], &axesCount);
		...
		for (size_t j = 0; j < axesCount; j++) {
			...
			case 1: // Left stick Y [1]
				...
				if (sDeviceAxesStates[i][0] != leftStickAxes.x || sDeviceAxesStates[i][1] != leftStickAxes.y) {
					InternalGamepadAxis(j, leftStickAxes, static_cast<QGamepad>(sGamepadIds[i]));
				}
				break;
			...
			case 3: // Right stick Y [3]
				...
				if (sDeviceAxesStates[i][2] != rightStickAxes.x || sDeviceAxesStates[i][3] != rightStickAxes.y) {
					InternalGamepadAxis(j, rightStickAxes, static_cast<QGamepad>(sGamepadIds[i]));
				}
				break;
			case 4: // L trigger [4]
			...
			case 5: // R trigger [5]
				...
				if (sDeviceAxesStates[i][4] != triggersAxes.x || sDeviceAxesStates[i][5] != triggersAxes.y) {
					InternalGamepadAxis(j, triggersAxes, static_cast<QGamepad>(sGamepadIds[i]));
				}
				break;
			}
		}

		sDeviceAxesStates[i][0] = leftStickAxes.x;
		sDeviceAxesStates[i][1] = leftStickAxes.y;

		sDeviceAxesStates[i][2] = rightStickAxes.x;
		sDeviceAxesStates[i][3] = rightStickAxes.y;

		sDeviceAxesStates[i][4] = triggersAxes.x;
		sDeviceAxesStates[i][5] = triggersAxes.y;
		...
		const unsigned char* const buttons = glfwGetJoystickButtons(sGamepadIds[i], &hatStatesCount);
		ASSERT(buttons, "Null device buttons!");

		for (size_t j = 0; j < hatStatesCount; j++) {
			if (sDeviceButtonsStates[i][j] != buttons[j]) {
				InternalGamepadButton(static_cast<QGamepad>(QGamepad::e_Gamepad0 + j), static_cast<QKeyState>(QKeyState::e_KeyStateDown == buttons[j]), static_cast<QGamepad>(sGamepadIds[i]));
			}
			sDeviceButtonsStates[i][j] = buttons[j];
		}
	}
}
...
void Shutdown() {
	...
	sGamepadIds.clear();
	sDeviceButtonsStates.clear();
	sDeviceAxesStates.clear();
}

bool KeyDown(const QKey aKey) {
	switch (aKey) {
	...
	return GLFW_PRESS == glfwGetKey(static_cast<GLFWwindow*>(Window::GetContext()), LocalQwerkEToGlfw(aKey));
}

bool MouseDown(const QKey aKey) {
	if (e_Any == aKey) {
		return sMouseButtons.DownKeys() > 0;
	}
	// #TODO GLFWindow* reference. Remember to test multi-window input
	// #TODO Handle invalid key. Maybe move mouse and joystick buttons into their own enum
	return GLFW_PRESS == glfwGetMouseButton(static_cast<GLFWwindow*>(Window::GetContext()), LocalQwerkEToGlfw(aKey));
}
...
bool GamepadDown(const QGamepad aKey, const QGamepad aGamepadId) {
	...
	if (e_GamepadAny == aKey) {
		for (u8 i = 0; i < sGamepadsButtons.size(); i++) {
			if (aGamepadId == sGamepadsButtons[i].first) {
				return sGamepadsButtons[i].second.DownKeys() > 0;
			}
		}
	}
	// #TODO GLFWindow* reference. Remember to test multi-window input
	sDeviceButtonsStates;

	if (QGamepad::e_GamepadAny == aKey) {
		for (u8 i = 0; i < sDeviceButtonsStates.size(); i++) {
			if (e_KeyStateDown == sDeviceButtonsStates[aGamepadId][i]) {
				return true;
			}
		}
		return false;
...
static void LocalGlfwKeyCallback(GLFWwindow* aWindow, const int aKeyCode, const int aScancode, const int aAction, const int aMode) {
	if (GLFW_REPEAT != aAction) // #TODO Review GLFW_REPEAT
	{
		const QKey qwerkEeKey = LocalGlfwToQwerkE(aKeyCode, aScancode);
		if (QKey::e_QKeyMax != qwerkEeKey) {
			InternalKeyChanged(qwerkEeKey, static_cast<QKeyState>(GLFW_PRESS == aAction));
		}

#ifdef _QDEARIMGUI
		ImGui_ImplGlfw_UpdateKeyModifiers(aWindow);
		// #NOTE ImGui_ImplGlfw_TranslateUntranslatedKey() contents were stripped by #ifs so removed entirely

		ImGuiKey imgui_key = ImGui_ImplGlfw_KeyToImGuiKey(aKeyCode, aScancode);
		ImGui::GetIO().AddKeyEvent(imgui_key, (aAction == GLFW_PRESS));
#endif // _QDEARIMGUI
	}
}

static void LocalGlfwCharCallback(GLFWwindow* aWindow, const unsigned int aCodePoint) {
#ifdef _QDEARIMGUI
	if (aCodePoint > 0 && aCodePoint < 0x10000) {
		ImGui::GetIO().AddInputCharacter(aCodePoint);
	}
#endif // _QDEARIMGUI
}

static void LocalGlfwCursorPositionCallback(GLFWwindow* aWindow, const double aPosX, const double aPosY) {
	InternalMouseMove({ aPosX, aPosY });

#ifdef _QDEARIMGUI
	ImGui::GetIO().AddMousePosEvent((float)aPosX, (float)aPosY);
#endif // _QDEARIMGUI
}

static void LocalGlfwMouseButtonCallback(GLFWwindow* aWindow, const int aButton, const int aAction, const int aMods) {
	const QKey qKey = LocalGlfwToQwerkE(aButton, 0);
	InternalMouseButton(qKey, static_cast<QKeyState>(GLFW_PRESS == aAction));

#ifdef _QDEARIMGUI
	ImGui_ImplGlfw_UpdateKeyModifiers(aWindow);

	if (aButton >= 0 && aButton < ImGuiMouseButton_COUNT) {
		ImGui::GetIO().AddMouseButtonEvent(aButton, aAction == GLFW_PRESS);
	}
#endif // _QDEARIMGUI
}

static void LocalGlfwScrollCallback(GLFWwindow* aWindow, const double aOffsetX, const double aOffsetY) {
	InternalMouseScroll(aOffsetX, aOffsetY);

#ifdef _QDEARIMGUI
	ImGui::GetIO().AddMouseWheelEvent(static_cast<float>(aOffsetX), static_cast<float>(aOffsetY));
#endif // _QDEARIMGUI
}

static void LocalGlfwJoystickCallback(const int aJoystickId, const int eventId) {
	...
	case GLFW_CONNECTED: // 262145
		AddGamepad(aJoystickId);
		break;

	case GLFW_DISCONNECTED: // 262146
		RemoveGamepad(aJoystickId);
		break;
...
void InputRegisterGlfwCallbacks(GLFWwindow* window) {
	...
	glfwSetKeyCallback(window, LocalGlfwKeyCallback);
	glfwSetCharCallback(window, LocalGlfwCharCallback);

	// For keys as char values: 'a'=97, 'A'=65, mod shift=1 or GLFW_MOD_SHIFT
	// glfwSetCharModsCallback(window, LocalGlfwCharModsCallback);

	glfwSetCursorPosCallback(window, LocalGlfwCursorPositionCallback);
	glfwSetMouseButtonCallback(window, LocalGlfwMouseButtonCallback);
	glfwSetScrollCallback(window, LocalGlfwScrollCallback);

	glfwSetJoystickCallback(LocalGlfwJoystickCallback);
}

void InputUnregisterGlfwCallbacks(GLFWwindow* window) {
	glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
...
static ImGuiKey ImGui_ImplGlfw_KeyToImGuiKey(const int keycode, const int scancode) {
	IM_UNUSED(scancode);
	switch (keycode) {
...
constexpr int LocalQwerkEToGlfw(const QGamepad aQwerkEKey) {
	switch (aQwerkEKey) // GLFW keys defined in glfw3.h, or https://www.glfw.org/docs/latest/group__keys.html
	{
...
constexpr int LocalQwerkEToGlfw(const QKey aQwerkEKey) {
	// GLFW keys defined in glfw3.h, or https://www.glfw.org/docs/latest/group__keys.html
	...
	// #TODO Any return value, or blocked/disabled/unsupported/default case with assert?
	...
	LOG_CRITICAL("Unsupported QKey {0}!", aQwerkEKey);
	// ASSERT(false, "Unsupported QKey key!");
	return QKey::e_QKeyMax;
}

constexpr QKey LocalGlfwToQwerkE(const int aGlfwKey, const int aScancode) {
	// GLFW keys defined in glfw3.h, or https://www.glfw.org/docs/latest/group__keys.html
	...
	case 256: // Max falcon keys: play/pause, volume up, volume down;
		LOG_CRITICAL("Unsupported media key scancode {0}!", aScancode);
		return QKey::e_QKeyMax;
	...
	LOG_CRITICAL("Unsupported GLFW key {0} or scancode {1}!", aGlfwKey, aScancode);
